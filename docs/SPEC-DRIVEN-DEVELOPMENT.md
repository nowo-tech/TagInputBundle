...
2. **Product behavior** — what **TagInputBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** form author, **I want** tag-style multi-input UI **so that** users add/remove tokens visually. |
| US-02 | **As an** integrator, **I want** one Symfony field storing comma-separated or JSON values **so that** backend validation stays simple. |
| US-03 | **As an** integrator, **I want** configurable delimiters and max tags **so that** the UX matches my domain (emails, skills, labels). |
| US-04 | **As a** maintainer, **I want** frontend tests in CI **so that** keyboard and paste interactions remain stable. |
...
**Goal:** Symfony Tag FormType with multi-input UI mapped to a single field value.
